"""CLI wrapper for deterministic synthetic robustness benchmark."""
...
from src.deterministic_mixing_inversion import run_synthetic_robustness_benchmark
...
DEFAULT_CONFIG = REPO_ROOT / "configs/deterministic_inversion/robustness_default.yaml"
DEBUG_CONFIG = REPO_ROOT / "configs/deterministic_inversion/robustness_debug.yaml"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run deterministic synthetic robustness benchmark and metric ranking."
    )
    parser.add_argument("--config", type=str, default=None, help="Path to YAML config.")
    parser.add_argument("--out_dir", type=str, default=None, help="Override output directory.")
    parser.add_argument("--candidate-root", type=str, default=None, help="Override candidate pool root dir.")
    parser.add_argument("--a-pattern", type=str, default=None, help="Regex for A-type pool entries.")
    parser.add_argument("--b-pattern", type=str, default=None, help="Regex for B-type pool entries.")
    parser.add_argument("--debug", action="store_true", help="Enable debug mode.")
    parser.add_argument("--sample-limit", type=int, default=None, help="Override max benchmark samples.")
    parser.add_argument(
        "--run-tag",
        type=str,
        default=None,
    ...
    parser.add_argument(
        ...
        default=[],
        metavar="key=value",
        help="Repeatable dot-path override, e.g. --set synthetic_benchmark.x_values='[0.25,0.5,0.75]'.",
    )
...
def build_overrides(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    if args.sample_limit is not None:
        overrides.setdefault("synthetic_benchmark", {})["max_samples"] = args.sample_limit
    return overrides
...
def main() -> None:
    ...
    config = deep_update(base_config, build_overrides(args))
    try:
    ...
    if args.run_tag:
        out_dir = Path(config.get("output", {}).get("out_dir", "outputs/deterministic_robustness"))
        config.setdefault("output", {})["out_dir"] = str(resolve_run_dir(out_dir, args.run_tag))
        config.setdefault("output", {})["run_tag"] = args.run_tag

    output_dir = Path(config.get("output", {}).get("out_dir", "outputs/deterministic_robustness"))
    ...
    try:
        logger.info("Starting synthetic robustness benchmark")
        summary = run_synthetic_robustness_benchmark(config=config, logger=logger)
        logger.info("Synthetic robustness benchmark complete")
    except Exception as exc:
        manifest["failures"].append({"error": str(exc)})
        logger.exception("Synthetic robustness benchmark failed")
        raise
    finally:
        ...
        logger.info(
            "Summary: records=%s | best_metric_by_mae=%s | runtime=%.2fs | out_dir=%s",
            summary.get("records"),
            summary.get("best_metric_by_mae"),
            wall_time,
            output_dir,
        )
