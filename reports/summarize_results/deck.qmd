...
comparison_metrics = report.get("comparison_metrics") or []
...
def _comparison_table(path_str, metrics_keys):
    if not path_str:
        return "_No comparison table available._"
    path = Path(path_str)
    if not path.exists():
        return "_Comparison table not found._"
    with path.open("r", newline="", encoding="utf-8") as handle:
        reader = csv.DictReader(handle)
        rows = list(reader)
    if not rows:
        return "_No comparison rows available._"
    base_cols = ["run_id", "stage", "status", "timestamp", "dataset", "epoch", "epochs_total"]
    columns = base_cols + list(metrics_keys or [])
...
```{python}
#| output: asis
print(_comparison_table(comparison_table, comparison_metrics))
```
